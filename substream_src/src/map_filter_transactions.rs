...
use substreams::store::{StoreSetInt64, StoreGetInt64};
...
fn map_filter_transactions(blk: Block) -> Result<CreatedTokens, Error> {
    ...
    for tx in blk.transactions.iter().filter(|tx| tx.meta.as_ref().map_or(false, |m|
        ...
        if let Some(trx) = confirmed_trx.transaction {
            ...
            if let (Some(msg), Some(meta)) = (trx.message, confirmed_trx.meta.as_ref()) {
                for (i, inst) in msg.instructions.iter().enumerate() {
                    ...
                    let maybe_token = inst.accounts.get(0)
                    ...
                    let params = vec![TokenParams { token_contract: token.clone(), token_decimals: 6.0 }];
